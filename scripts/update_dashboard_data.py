#!/usr/bin/env python3
"""Refresh dashboard data while preserving combined cash-flow-aware portfolio history."""
...
def _update_notice(prices):
    ...
    when=newest.astimezone(ZoneInfo("Europe/Vienna")).strftime("%d.%m.%Y, %H:%M Uhr"); positive=sum(float(i.get("price",0) or 0)>0 for i in rows); fresh=sum(i.get("status")=="fresh" for i in rows); stale=sum(i.get("status")=="stale" for i in rows); fallback=sum(i.get("status")=="fallback" for i in rows); warnings=len(prices.get("warnings",[]))
...
def _normalize_history(points):
    by={}
    for p in points:
        try:d=str(p.get("date",""));_date_key(d);row=dict(p);row["date"]=d;row["value"]=float(p["value"]);by[d]=row
...
def _enrich_cashflows(history):
    ...
    for p in _normalize_history(history):
        c=_net_contributions_at(_date_key(p["date"]),flows);v=float(p["value"]);g=round(v-c,2);row=dict(p);row["net_contributions"]=c;row["gain"]=g;row["simple_return"]=round(g/c,8) if c>0 else None;out.append(row)
    return out
...
def refresh_document(index_text,portfolio,prices,history=None):
    ...
    history=_enrich_cashflows(history);previous=_previous_total(history,asof,data.get("meta",{}).get("previousTotal",data.get("meta",{}).get("total",total)));by={p["date"]:dict(p) for p in history};by[asof]={"date":asof,"value":total,"validated":True};history=_enrich_cashflows(list(by.values()))
