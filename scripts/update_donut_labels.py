#!/usr/bin/env python3
"""Render readable external labels and tooltips for dashboard donut charts."""
...
JS = r'''function donut(el,legend,rows,total){
 ...
 const paths=segs.map(x=>`<path class="donut-segment" d="${arc(x.a0,x.a1)}" fill="${colors[x.i%colors.length]}" stroke="#141b2d" stroke-width="1"><title>${x.name} · ${pct.format(x.share)}</title></path>`).join('');
 const callouts=calls.map(x=>{const elbow=polar(ro+26,x.mid),tx=x.side==='r'?W-18:18,ex=x.side==='r'?W-118:118,anchor=x.side==='r'?'end':'start',name=String(x.name).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');return `<g class="donut-callout"><polyline points="${x.edge.x},${x.edge.y} ${elbow.x},${elbow.y} ${ex},${x.ly}" fill="none" stroke="${colors[x.i%colors.length]}" stroke-width="1.4"/><text x="${tx}" y="${x.ly-3}" text-anchor="${anchor}"><tspan class="pct-label">${pct.format(x.share)}</tspan><tspan class="name-label" x="${tx}" dy="14">${name}</tspan></text></g>`}).join('');
...
def main():
    ...
    fn = re.compile(r'function donut\(el,legend,rows,total\)\{[^\n]*\}')
    if not fn.search(text):
        raise ValueError('Could not locate donut renderer')
    text = fn.sub(JS, text, count=1)
    text = re.sub(r"donut\('assetDonut','assetLegend',DATA\.assets,DATA\.meta\.total\);", "donut('assetDonut','assetLegend',DATA.assets,DATA.meta.total);", text, count=1)
    ...
    print('Updated Asset Allocation and Branchenanteile donuts with 8 external labels and hover tooltips.')
