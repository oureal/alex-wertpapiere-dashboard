...
def update_prices(instruments, mappings, existing, provider, now=None):
    ...
    for instrument in instruments:
        ...
        if mapping.get("enabled_for_test") and mapping.get("symbol"):
            ...
            warnings.append(f"{instrument_id}: no enabled Yahoo mapping; official/legacy fallback required")
...
def main() -> int:
    ...
    result = update_prices(instruments, mappings, existing, YFinanceProvider())
            ...
            "instruments": [
                {
                    ...
                    **price_by_id[item["id"]],
                    "automatable": price_by_id[item["id"]]["provider"] == "yfinance",
                }
    ...
    if args.discovery_output:
        ...
        args.discovery_output.write_text(json.dumps({"schema_version": 1, "provider": "yfinance", "purpose": "runtime_validation", "mappings": mappings["mappings"]}, ensure_ascii=False, indent=2) + "\n")
    fresh = sum(item["status"] == "fresh" for item in result["prices"])
    print(f"Yahoo update complete: {fresh} fresh, {len(result['prices']) - fresh} stale/fallback")
    return 0
